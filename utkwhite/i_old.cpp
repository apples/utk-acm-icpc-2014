...
for(int j=0;j<visited.size();j++)
	...
	if(room[x][y]>=1 &&x!=X-1&&room[x+1][y]==0&&room[x][y]<i)
	{
	room[x+1][y]=i;
	wet_spots++;
	 added=true;
	...
	if(room[x][y]>=1 &&x!=0&&room[x-1][y]==0&&room[x][y]<i)
	{
	room[x-1][y]=i;
	wet_spots++;
	 added=true;
	}
	if(room[x][y]>=1 &&y!=0&&room[x][y-1]==0&&room[x][y]<i)
	{
	room[x][y-1]=i;
	wet_spots++;
	 added=true;
	}
	if(room[x][y]>=1 &&y!=Y-1&&room[x][y+1]==0&&room[x][y]<i)
	{
	room[x][y+1]=i;
	wet_spots++;
	 added=true;
